...
const Folder = forwardRef<HTMLDivElement, FolderProps>(function Folder({ color, label, onClick, onMouseEnter }, ref) {
  return (
    <div
      ...
      className="flex flex-col items-center gap-2.5 sm:gap-4 cursor-pointer transition-transform duration-100 hover:-translate-y-2 hover:scale-105"
    >
      {/* eslint-disable-next-line @next/next/no-img-element -- pixel art must not be resampled */}
      <img src={"/folders/folder-" + color + ".png"} alt={label} className="w-[84px] sm:w-[110px] md:w-[120px]" style={{ imageRendering: "pixelated" }} />
      <div className="font-pixel text-[9px] sm:text-xs text-center" style={{ color: LABEL_COLOR[color] }}>{label}</div>
    </div>
  );
});
