...
export default function CtfCard({ item }: { item: CtfItem }) {
  ...
  return (
    <div className="bg-card border-[3px] border-ink shadow-pixelLg flex flex-col">
      {item.image && (
        <div className={"border-b-[3px] border-ink overflow-hidden " + (item.imageContain ? "bg-bg" : "h-[220px]")}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={item.image}
            alt={item.title}
            className={item.imageContain ? "w-full h-auto block" : "w-full h-full object-cover"}
          />
        </div>
      )}
